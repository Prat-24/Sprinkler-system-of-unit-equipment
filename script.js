document.getElementById("saveAsPdf").addEventListener("click", function(event) {
  event.preventDefault(); // Prevent form submission

  // Collect form data
  const formData = {
      Shift:document.querySelector('select[name="Shift"]').value,
      Date:document.querySelector('input[name="Date"]').value,
      Unitarea:document.querySelector('input[name="Unitarea"]').value,
      Testingpermission:document.querySelector('input[name="Testingpermission"]').value,

      // New fields added
      EquipmentNO: document.querySelector('input[name="Equipment"]').value,
      Unit: document.querySelector('input[name="unit"]').value,
      Observations: document.querySelector('input[name="Observations"]').value,
      NotificationNo: document.querySelector('input[name="NotificationNo"]').value,
      Status: document.querySelector('input[name="Status"]').value,
      Typeofdefects: document.querySelector('input[name="Typeofdefects"]').value,
      Assignedto: document.querySelector('input[name="Assignedto"]').value,
      Lastchecked: document.querySelector('input[name="Lastchecked"]').value,
      Nextduedate: document.querySelector('input[name="NextDuedate"]').value,
      //ETC: document.querySelector('input[name="ETC"]').value,
      
      // Add the new fields for the textarea elements
      Lineleak: document.querySelector('textarea[name="Lineleak"]').value,
      nozzleschocked: document.querySelector('textarea[name="nozzleschocked"]').value,
      otherobservations: document.querySelector('textarea[name="otherobservations"]').value,
       // New fields added
       Maintenance: document.querySelector('textarea[name="Maintenance"]').value,
      Shiftofficername: document.querySelector('input[name="Shiftofficername"]').value,
      OperatorName: document.querySelector('input[name="OperatorName"]').value // Note: Use an underscore instead of a space in the key
    };
    
    

  // Create a new PDF document
  const { jsPDF } = window.jspdf;
  const pdf = new jsPDF();




  // Define the border dimensions
const margin = 5; // Margin for the border
const pageWidth = pdf.internal.pageSize.getWidth();
const pageHeight = pdf.internal.pageSize.getHeight();

// Set border color (e.g., black)
pdf.setDrawColor(0, 0, 0); // Set draw color to black
pdf.setLineWidth(0.5); // Set line width for the border

// Draw the border rectangle
pdf.rect(margin, margin, pageWidth - 2 * margin, pageHeight - 2 * margin); // Draw border

 // Function to add header
 function addHeader() {
  const titleY = 30;
  const headingY = 20;
  const subtitleY = 35; // Adjusted position for subtitle
  const gap = 10; // Adjust this value for desired gap

  
  // Draw box for the title
  pdf.setFillColor(173, 216, 230);
  pdf.rect(5, headingY - 10, 20, 30, 'S'); // Valid parameters: x, y, width, height, style
  pdf.setFontSize(8);
  // Add image to PDF
  const imageData = 'data:image/png;base64,'; // Truncated for brevity
  pdf.addImage(imageData, 'PNG', 5, 10.3, 20, 29.6); // Adjust x, y, width, height

  //pdf.text('HP', 20, headingY + 5, { align: 'center' }); // Placeholder logo text
  
     // Draw main heading box
     pdf.setFillColor(255, 255, 255); // White color
pdf.rect(25, headingY - 10, 130, 25, 'FD'); // Ensure all arguments are valid
pdf.setFontSize(12);
pdf.text('Hindustan Petroleum Corporation Limited', 95, headingY, { align: 'center' });
pdf.text('Mumbai Refinery', 95, headingY + 5, { align: 'center' });

   // Draw subtitle box
pdf.setFillColor(255,255,255); // Light blue color
pdf.rect(25, subtitleY - 4, 130, 8.8, 'FD'); // Subtitle box
pdf.setFontSize(12);
pdf.text('Testing Of Sprinkler System of Unit Equipment', 95, subtitleY + 1, { align: 'center' });


// Set fill color to white
pdf.setFillColor(255, 255, 255); // White color

// Draw rectangular box on the right side
const boxWidth = 52;  // Width of the rectangular box
const boxHeight = 30; // Height of the rectangular box
const boxX = 153;     // Position from the left edge (right side of the page)
const boxY = 9.8; // Position vertically, adjusting with titleY

// Create the rectangular box on the right side of the page
pdf.rect(boxX, boxY, boxWidth, boxHeight, 'FD'); // 'FD' means fill and draw

// Draw metadata text inside the box (aligned to the right)
pdf.setFontSize(8);

// Position the text within the square box
const textX = boxX + 1 ; // X position for text, 10 units from the left of the box
const textY = boxY + 3.5; // Starting Y position for text inside the box

pdf.text('Issue No: 1', textX, textY);
pdf.text('Issue Date: 6/11/2024', textX, textY + 5);
pdf.text('Rev No: 0', textX, textY + 10);
pdf.text('Rev Date:', textX, textY + 15);
pdf.text('Doc No: HPCL MR/FRM/F&S/08200-08', textX, textY + 20);

// Adjust position for next elements if needed
// Example of increasing position for further content

  // Return updated position for fields
  return subtitleY + 4 + gap; // Move down for the next row with gap
}


// Function to add fields
function fieldPair(label1, value1, label2, value2, positionY) {
    const label1Width = 42;
    const value1Width = 45;
    const label2Width = 42;
    const value2Width = 60;

// Define a left margin
const leftMargin = 14;

// Calculate the positions for the label and value
const xLabelCenter = leftMargin; // Start from the left margin
const xValueCenter = xLabelCenter + label1Width; // Adjust to place value box directly after label box

// Draw joined label and value boxes
pdf.setDrawColor(0); // Set draw color for boxes
pdf.setFillColor(255, 255, 255); // Set fill color for boxes
pdf.rect(xLabelCenter - 5, positionY - 8, label1Width, 10); // Label box
pdf.rect(xValueCenter - 5, positionY - 8, value1Width, 10); // Value box

// Define label text position with vertical adjustment for centering

const textVerticalOffset = 2; // Adjust this based on font style and size
// Define label text position with vertical adjustment for centering
const label1FontSize = 11; // Increased font size for label1


// Set font and size
pdf.setFont('Arial', 'B');
pdf.setFontSize(label1FontSize);
// Draw label text, with center alignment adjustment for vertical positioning
pdf.text(label1, xLabelCenter, positionY - textVerticalOffset);

// Add value text
pdf.setFont('Arial', '', 12);
pdf.text(value1, xValueCenter , positionY - textVerticalOffset); // Add a small offset for value text


// Draw second label and value if provided
if (label2) {
    const xLabel2Center = 101; // Adjust to place second label box directly after first value box
    const xValue2Center = 143; // Adjust for second label box

    // Draw joined second label and value boxes
    pdf.rect(xLabel2Center - 5, positionY - 8, label2Width, 10); // Second label box
    pdf.rect(xValue2Center - 5, positionY - 8, value2Width, 10); // Second value box

  
    const textVerticalOffset = 2; // Adjust this based on font style and size
    
    // Define label text position with vertical adjustment for centering
const label2FontSize = 11; // Increased font size for label1


// Set font and size
pdf.setFont('Arial', 'B');
pdf.setFontSize(label2FontSize);
    
    // Draw label text, with center alignment adjustment for vertical positioning
    pdf.text(label2, xLabel2Center, positionY - textVerticalOffset);
    
        // Add second value text
        pdf.setFont('Arial', '', 12);
        pdf.text(value2, xValue2Center , positionY - textVerticalOffset); // Add a small offset for value text
    
    
    
}

return positionY + 12; // Move down for the next row and return updated positionY
}

  // Add data to the PDF
  let positionY = addHeader(); // Add header for the page and get the new position
  positionY += 10; // Additional gap if needed

  // Create an array from the form data to loop through
  const data = [formData]; // Wrap formData in an array for processing
  for (const row of data) {
    let positionY = addHeader(); // Add header for the page and get the new position
                    positionY += 10; // Additional gap if needed
                    
                    positionY = fieldPair('Shift:', row.Shift, 'Date:', row.Date, positionY);
                    positionY = fieldPair( 'Unit area:', row.Unitarea, 'Testing permission:', row.Testingpermission, positionY);

                    positionY += 8; // Additional gap if needed
                    positionY = fieldPair('Equipment No:', row.EquipmentNO, 'Unit:',row.Unit, positionY);
                    positionY = fieldPair('Status:', row.Status,'Observations:', row.Observations, positionY);
                    positionY = fieldPair('NotificationNo:', row.NotificationNo,'Type of defects:', row.Typeofdefects,  positionY);
                   positionY = fieldPair('Assigned to:', row.Assignedto,'Last  checked:', row.Lastchecked,  positionY);
                    positionY = fieldPair( 'Next due date:', row.Nextduedate,'', '',  positionY);
                    
                   
                    positionY += 4; 
    // Add the "Observations during testing" header
const observationsHeader = 'OBSERVATIONS DURING TESTING';
pdf.setFont('Arial', 'B', 20);
pdf.setTextColor(0, 0, 0); // RGB for black
const observationsTextWidth = pdf.getTextWidth(observationsHeader);
// Set the x position to the left margin (adjust as needed)
const xObservations = 10; // Replace 20 with your desired left margin value
pdf.text(observationsHeader, xObservations, positionY); // Align the header to the left
positionY += 0; // Add some spacing

// Create a box for "Line leak"
//pdf.rect(10, positionY, 50, 20); // Adjust dimensions as needed
pdf.setFont('Arial', '', 12);
pdf.text('a.Line leak (Yes/No):-', 30, positionY + 15, { align: 'center' }); // Adjust x and y positions as needed

// Add the textarea for "Line leak"
//pdf.rect(60, positionY, 135, 20); // Adjust dimensions as needed
pdf.setFont('Arial', '', 12);
pdf.text(row.Lineleak, 130, positionY + 15, { align: 'center' }); // Adjust x and y positions as needed

// Add spacing
positionY += 10;

// Create a box for "No of Nozzles Chocked"
pdf.rect(9, positionY, 54, 20);
pdf.setFont('Arial', '', 12);
pdf.text('b.No of Nozzles Chocked:-', 34, positionY + 15, { align: 'center' });

// Add the textarea for "No of Nozzles Chocked"
pdf.rect(63, positionY, 135, 20);
pdf.setFont('Arial', '', 12);
pdf.text(row.nozzleschocked, 130, positionY + 15, { align: 'center' });

// Add spacing
positionY += 10;

// Create a box for "Any other observations"
pdf.rect(9, positionY, 54, 40);
pdf.setFont('Arial', '', 12);
pdf.text('c.Any other observations:-', 34, positionY + 15, { align: 'center' });

// Add the textarea for "Any other observations"
pdf.rect(63, positionY, 135, 40);
pdf.setFont('Arial', '', 12);
pdf.text(row.otherobservations, 130, positionY + 15, { align: 'center' });



 // Start a new page
 //{
  //pdf.addPage();
  //positionY = addHeader() + 20; // Reset positionY after adding a header on the new page

//}

//positionY = addHeader() + 20; 

// Draw the border with a margin
const margin = 5; // Adjust the margin value as needed
pdf.setLineWidth(0.5);
pdf.setTextColor(0, 0, 0); // RGB for red
pdf.rect(margin, margin, pageWidth - 2 * margin, pageHeight - 2 * margin);
pdf.stroke();
// Add the "Maintenance Follow Up Required" text at the top of the second page
const x = 10; // Adjust the value as needed
const y = 250;
const maintenanceText = 'MAINTENANCE FOLLOW UP REQUIRED';
const maintenanceFontSize = 11; // Set the desired font size for "Remarks"

// Set the font size and add "Remarks" text
pdf.setFontSize(maintenanceFontSize);


// Add the text outside the box, above it
const xText = 10;
pdf.text(maintenanceText, xText, y - 20); // Adjust the x and y offsets as needed


// Create the textarea
const textareaX = 10; // Adjust the x offset as needed
const textareaY = 235;
const textareaWidth = 188; // Adjust the width as needed
const textareaHeight = 20; // Adjust the height as needed
pdf.rect(textareaX, textareaY, textareaWidth, textareaHeight);

// Add the maintenance value to the textarea
const maintenanceValue = document.querySelector('textarea[name="Maintenance"]').value;
pdf.text(maintenanceValue, textareaX + 5, textareaY + 15); // Adjust the x and y offsets as needed



// Add the "Tested By" text after Petroleum class
positionY= 266;
const testedByText = 'TESTED BY';
const testedByFontSize = 11; // Set the desired font size for "Remarks"

// Set the font size and add "Remarks" text
pdf.setFontSize(testedByFontSize);

pdf.setFont('Arial', 'B', 18);
pdf.setTextColor(0, 0, 0); // RGB for black
const textWidth = pdf.getTextWidth(testedByText);
const xCenter = (pageWidth - textWidth) / 2; // Calculate center position
pdf.text('TESTED BY:', 10, positionY); // Adjust the x and y positions as needed
positionY += 15; // Move down for the next field (adjust if necessary)
positionY = fieldPair('Shift Officer Name:', row.Shiftofficername,'Operator Name:',row.OperatorName, positionY);
  }

  // Add Footer
  const totalPages = pdf.internal.getNumberOfPages();
  for (let i = 1; i <= totalPages; i++) {
      pdf.setPage(i);
      pdf.setFontSize(8);
      pdf.text(`Page ${i} of ${totalPages}`, pdf.internal.pageSize.width / 2, pdf.internal.pageSize.height - 10, { align: 'center' });
  }

  // Save the PDF
  pdf.save('Unit_equipment.pdf');
});

  